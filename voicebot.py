...
def main():
  ...
  ##################################################################
  ##2. 상태를 저장하기 위한 session_state추가
  ##2-1 session state초기화
  ##2-2 리셋코드
  ###############################################################
  #2-1 session state초기화
  ...
  with st.sidebar:
    # OpenAI API Key 입력받기
    openai.api_key = st.text_input(label = 'OpenAI API Key', placeholder = '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', value = '', type = 'password')
    ...
    if st.button(label = '초기화'):
      ...
      #2-2리셋 코드
      ######################################################################
      #리셋 버튼을 누르면 기존 대화 내용을 모두 삭제하기 위해 st.sessionn_state['chat']과
      #st.session_state['message']의 session_state를 초기화한다.

      st.session_state["chat"] = []
      st.session_state["messages"] = [{"role": "system", "content": "You are a thoughtful assistance. Respond to all"}]
  ...
  #기능구현공간
  ###################################################################################################################
  #기능구현 영역을 두개의 영역으로 분할한다. 분할한 각 영역의 이름은 col1, col2로 지정한다.
